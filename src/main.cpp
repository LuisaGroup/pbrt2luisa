...
namespace minipbrt {
    ...
    static void print_camera(const Camera *camera) {
        ...
        switch (camera->type()) {
            case CameraType::Perspective: {
                ...
                printf("screenwindow     = [ %f, %f, %f, %f ]\n", typedCam->screenwindow[0], typedCam->screenwindow[1],
                       typedCam->screenwindow[2], typedCam->screenwindow[3]);
                printf("lensradius       = %f\n", typedCam->lensradius);
    ...
    static void print_film(const Film *film) {
        ...
        switch (film->type()) {
            case FilmType::Image: {
                ...
                printf("cropwindow         = [ %f, %f, %f, %f ]\n", imagefilm->cropwindow[0],
                       imagefilm->cropwindow[1], imagefilm->cropwindow[2], imagefilm->cropwindow[3]);
                printf("scale              = %f\n", imagefilm->scale);
    ...
    static void print_integrator(const Integrator *integrator) {
        static const char *integratorTypes[] = {
            "bdpt", "directlighting", "mlt", "path", "sppm", "whitted", "volpath", "ambientocclusion", nullptr
        };
        ...
        switch (integrator->type()) {
            case IntegratorType::BDPT: {
                ...
                printf("maxdepth            = %d\n", typed->maxdepth);
                printf("pixelbounds         = [ %d, %d, %d, %d ]\n", typed->pixelbounds[0], typed->pixelbounds[1],
                       typed->pixelbounds[2], typed->pixelbounds[3]);
    ...
    static void print_sampler(const Sampler *sampler) {
        static const char *samplerTypes[] = {
            "02sequence", "lowdiscrepancy", "halton", "maxmindist", "random", "sobol", "stratified", nullptr
        };
    ...
    static void print_shapes_summary(const Scene *scene) {
        ...
        const char *names[] = {
            "cones", "curves", "cylinders", "disks", "hyperboloids", "paraboloids", "spheres", "trianglemeshes",
            "heightfields", "loopsubdivs", "nurbses", "plymeshes", nullptr
        };
        printf("==== Shape Types ====\n");
    ...
    static void print_materials_summary(const Scene *scene) {
        ...
        const char *names[] = {
            "disney", "fourier", "glass", "hair", "kdsubsurface", "matte", "metal", "mirror", "mix", "none", "plastic",
            "substrate", "subsurface", "translucent", "uber", nullptr
        };
    ...
    static void print_textures_summary(const Scene *scene) {
        ...
        const char *names[] = {
            "bilerp", "checkerboard", "constant", "dots", "fbm", "imagemap", "marble", "mix", "scale", "uv", "windy",
            "wrinkled", "ptex", nullptr
        };
    ...
    if (filenames.empty()) {
        ...
        for (const std::string &filename: filenames) {
            ...
            if (!ok) {
                ...
                printf(" ---> [%s, line %u, column %u] %s\n", err->filename(), uint32_t(err->line()),
                       uint32_t(err->column()), err->message());
                ++numFailed;
            } else if (!plyOK) {
